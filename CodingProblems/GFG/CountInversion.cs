...
namespace CodingProblems.GFG
{
    public class CountInversion
    {
        ...
        {
            long tempTruckSize = truckSize;
            List<(long, long)> tupple = new List<(long, long)>();
            for (int i = 0; i < unitsPerBox.Count; i++)
            {
                tupple.Add((unitsPerBox[i], boxes[i]));
            }
            tupple.Sort((a, b) =>
                        b.Item1 == a.Item1 ? (b.Item2 - a.Item2 > 0 ? 1 : -1) : (b.Item1 - a.Item1 > 0 ? 1 : -1));
            ...
            //                    ? b : a));
            long ans = 0;
            for (int i = 0; i < unitsPerBox.Count && tempTruckSize != 0; i++){

                long units = tupple[i].Item1;
                long noOfBox = tupple[i].Item2;

                if (noOfBox <= tempTruckSize){
                    ans += (units * noOfBox);
                    tempTruckSize -= noOfBox;
                } else {
                    long tBox = noOfBox - tempTruckSize;
                    ans += (units * tBox);
                    tempTruckSize = 0;
                }
            }
